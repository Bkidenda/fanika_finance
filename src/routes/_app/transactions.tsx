...
import { Plus, Search, Trash2, AlertTriangle, Pencil, ArrowDownRight, ArrowUpRight } from "lucide-react";
...
// Unified ledger row (income entry or expense) used for listing, filtering & totals
type Row = {
  id: string; kind: Kind; date: string; amount: number; fee: number;
  category: string; description: string | null; account_id: string | null;
  payment_method: string | null; is_emergency: boolean;
...
type EditState =
  | { kind: "expense"; id: string; date: string; amount: string; category: string; description: string; account_id: string; is_emergency: boolean; transaction_fee: string }
  | { kind: "income"; id: string; date: string; amount: string; source: string; account_id: string }
  | null;
...
function TransactionsPage() {
  ...
  const loading = expenses.isLoading || income.isLoading;
  ...
  const [incDate, setIncDate] = useState(isoLocalDate());
  ...
  async function saveIncome(e: React.FormEvent) {
    ...
    const { error } = await supabase.from("income_entries").insert({
      user_id: user.id, date: incDate, source: incSource || "Income", amount: amt,
  ...
  async function remove(row: Row) {
    ...
    if (error) return toast.error(error.message);
    toast.success(row.kind === "income" ? "Income entry deleted" : "Expense deleted");
    invalidateAll();
  }
  ...
  async function saveEdit(e: React.FormEvent) {
    ...
    if (edit.kind === "expense") {
      ...
      const { error } = await supabase.from("income_entries").update({
        date: edit.date, amount: Number(edit.amount), source: edit.source || "Income",
  ...
  const rows = useMemo<Row[]>(() => {
    const inc: Row[] = (income.data ?? []).map((e) => ({
      id: e.id, kind: "income", date: e.date, amount: Number(e.amount), fee: 0,
      category: "Income", description: e.source, account_id: e.account_id,
      payment_method: null, is_emergency: false, source_debt_payment_id: null,
    }));
    const exp: Row[] = (expenses.data ?? []).map((e) => ({
      id: e.id, kind: "expense", date: e.date, amount: Number(e.amount), fee: Number(e.transaction_fee ?? 0),
      category: e.category, description: e.description, account_id: e.account_id,
      payment_method: e.payment_method, is_emergency: e.is_emergency,
      source_debt_payment_id: e.source_debt_payment_id,
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Dialog open={incomeOpen} onOpenChange={setIncomeOpen}>
            <DialogTrigger asChild><Button variant="outline"><Plus className="mr-1 h-4 w-4" /> Add income</Button></DialogTrigger>
            <DialogContent>
              <DialogHeader><DialogTitle>Add income</DialogTitle></DialogHeader>
              <form onSubmit={saveIncome} className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5"><Label>Date</Label><Input type="date" value={incDate} onChange={(e) => setIncDate(e.target.value)} required /></div>
                  <div className="space-y-1.5"><Label>Amount ({currency})</Label><Input type="number" min="0" step="0.01" value={incAmount} onChange={(e) => setIncAmount(e.target.value)} required /></div>
                </div>
                <div className="space-y-1.5"><Label>Source</Label><Input value={incSource} onChange={(e) => setIncSource(e.target.value)} placeholder="Salary, business…" /></div>
      ...
      <div className="rounded-2xl border bg-card shadow-card">
        ...
        {loading ? (
          ...
          <WeeklyLedger
            ...
            onEdit={(r) => setEdit(
              r.kind === "expense"
                ? { kind: "expense", id: r.id, date: r.date, amount: String(r.amount), category: r.category, description: r.description ?? "", account_id: r.account_id ?? "", is_emergency: r.is_emergency, transaction_fee: String(r.fee ?? 0) }
                : { kind: "income", id: r.id, date: r.date, amount: String(r.amount), source: r.description ?? "", account_id: r.account_id ?? "" }
            )}
            onRemove={remove}
          />
        ) : <p className="py-10 text-center text-sm text-muted-foreground">No transactions recorded yet.</p>}
      </div>

      <Dialog open={!!edit} onOpenChange={(o) => !o && setEdit(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>{edit?.kind === "income" ? "Edit income" : "Edit expense"}</DialogTitle></DialogHeader>
          {edit && edit.kind === "expense" && (
          ...
          {edit && edit.kind === "income" && (
            <form onSubmit={saveEdit} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5"><Label>Date</Label><Input type="date" value={edit.date} onChange={(ev) => setEdit({ ...edit, date: ev.target.value })} /></div>
                <div className="space-y-1.5"><Label>Amount</Label><Input type="number" step="0.01" value={edit.amount} onChange={(ev) => setEdit({ ...edit, amount: ev.target.value })} /></div>
              </div>
              <div className="space-y-1.5"><Label>Source</Label><Input value={edit.source} onChange={(ev) => setEdit({ ...edit, source: ev.target.value })} /></div>
              <div className="space-y-1.5"><Label>Account</Label>
                <Select value={edit.account_id} onValueChange={(v) => setEdit({ ...edit, account_id: v })}>
                  <SelectTrigger><SelectValue placeholder="(none)" /></SelectTrigger>
                  <SelectContent>{(accounts.data ?? []).map((a) => <SelectItem key={a.id} value={a.id}>{a.name}</SelectItem>)}</SelectContent>
...
function WeeklyLedger({ items, currency, onEdit, onRemove }: {
  items: Row[]; currency: string;
  onEdit: (r: Row) => void; onRemove: (r: Row) => void;
}) {
  const [openWeeks, setOpenWeeks] = useState<Record<string, boolean>>({});
  // Group by week, latest first
  const groups = new Map<string, { label: string; start: Date; rows: Row[] }>();
  items.forEach((e) => {
  ...
  return (
    <div className="divide-y">
      {sorted.map(([key, g], idx) => {
        ...
        const netTotal = g.rows.reduce((s, r) => s + (r.kind === "income" ? r.amount : -(r.amount + r.fee)), 0);
        ...
        return (
          <div key={key}>
            ...
            >
              ...
              <span className={`tabular-nums ${netTotal >= 0 ? "text-success" : ""}`}>{netTotal >= 0 ? "+" : "−"}{formatCurrency(Math.abs(netTotal), currency)} {isOpen ? "▾" : "▸"}</span>
            </button>
            {isOpen && days.map(([day, dayRows]) => (
              <div key={day}>
                ...
                {dayRows.map((e) => {
                  ...
                  return (
                    <div key={`${e.kind}-${e.id}`} className="flex items-center justify-between px-4 py-2.5 text-sm hover:bg-secondary/40">
                      <div className="flex min-w-0 items-center gap-2.5">
                        <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl ${e.kind === "income" ? "bg-success/15 text-success" : "bg-secondary text-secondary-foreground"}`}>
                          {e.kind === "income" ? <ArrowDownRight className="h-4 w-4" /> : <ArrowUpRight className="h-4 w-4" />}
                        </span>
                        <div className="min-w-0">
                          ...
                          <div className="text-[11px] text-muted-foreground">
                            {time && <>{time} · </>}{e.category}{e.payment_method ? ` · ${e.payment_method}` : ""}{e.fee > 0 ? ` · fee ${formatCurrency(e.fee, currency)}` : ""}
                          </div>
                        </div>
                      </div>
                      <div className="flex shrink-0 items-center gap-2.5">
                        <span className={`tabular-nums font-medium ${e.kind === "income" ? "text-success" : ""}`}>{e.kind === "income" ? "+" : "−"}{formatCurrency(Number(e.amount), currency)}</span>
                        {!e.source_debt_payment_id && (
                          <button onClick={() => onEdit(e)} aria-label="Edit" className="text-muted-foreground hover:text-primary"><Pencil className="h-3.5 w-3.5" /></button>
                        )}
                        <button onClick={() => onRemove(e)} aria-label="Delete" className="text-muted-foreground hover:text-destructive"><Trash2 className="h-3.5 w-3.5" /></button>
                      </div>
                    </div>
                  );
                })}
